...
stageConfigurations.forEach(stageConfig => {
  ...
  const globalResourcesStack = new GlobalResourcesStack(app, `${projectName}-frontend-global-resources-${nVirginia}-${stageName}`, {
    env: { region: nVirginia, account: account },
    crossRegionReferences: true,
    stageConfig: stageConfig
  });
  
  new InfraStack(app, `${projectName}-frontend-${stockholm}-${stageName}`, {
